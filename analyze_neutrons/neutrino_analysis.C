  ...
  void neutrino_analysis(){
    ...
    h_truePrCosAlign	     = new TH1D("truePrCosAlign","(n, 1p) Cos(Angle) B/w Inferred Primary Neutron Direction and True Proton Momentum;Absolute[Cos(angle)]",50,0,1);
    ...
    h_trueSignalX            = new TH1D("trueSignalX","True Particle Signal X Position;X [cm]",30,-25,275);
    h_blipSignalTrueX        = new TH1D("blipSignalTrueX","Blip Signal True X Position;X [cm]",30,-25,275);
    ...
    h_prTrksPerE	     = new TH1D("prTrksPerE", "Number of Reconstructed Objects vs True Energy of Protons From Primary Neutrons;True Proton Kinetic Energy [MeV];Number of Reconstructed Objects",100,0,200);
    h_prBlipsPerE            = new TH1D("prBlipsPerE","Number of Reconstructed Objects vs True Energy of Protons From Primary Neutrons;True Proton Kinetic Energy [MeV];Number of Reconstructed Objects",100,0,200);
    ...
    h_trueNeutronEvsProtonE  = new TH2D("trueNeutronEvsProtonE","True Proton Energy vs Neutron Energy;Primary Neutron Kinetic Energy [MeV];Proton Kinetic Energy [MeV]",60,0,300,60,0,300);
    ...
    h_pr_cos_vs_energy       = new TH2D("pr_cos_vs_energy","True Cos(angle) vs True (n, 1p) Energy (Single Proton from Primary Neutron);True Proton Kinetic Energy [MeV];cos(ang)",200,0,200,102,-1.02,1.02);
    h_pr_E_ds		     = new TH2D("pr_E_ds","Proton Blip Energy Deposition Density;Reconstructed Blip Energy (E_{blip}) [MeVee]; E_{blip}/ds [MeV/cm]",500,0,10,250,0,5);//150,0,15,120,0,12);
    gr_signal_E_ds = new TGraph(); gr_signal_E_ds->SetNameTitle("signal_E_ds", "Proton Blip Energy Deposition Density;Reconstructed Blip Energy (E_{blip}) [MeVee];E_{blip}/ds [MeV/cm]");
    h_elec_E_ds              = new TH2D("elec_E_ds","Electron Blip Energy Deposition Density;Reconstructed Blip Energy (E_{blip}) [MeVee]; E_{blip}/ds [MeV/cm]",500,0,10,250,0,5);
    gr_elec_E_ds = new TGraph(); gr_elec_E_ds->SetNameTitle("gr_elec_E_ds", "Electron Blip Energy Deposition Density;Reconstructed Blip Energy (E_{blip}) [MeVee];E_{blip}/ds [MeV/cm]");
    gr_cosmic_E_ds = new TGraph(); gr_cosmic_E_ds->SetNameTitle("cosmic_E_ds", "Blip Energy Deposition Density;Reconstructed Blip Energy (E_{blip}) [MeVee];E_{blip}/ds [MeV/cm]");
   ...
   // outfile->cd();
	...
	if( part_process->at(i) == "nCapture" ) { ncapture_products_g4id.push_back(g4id); }
	  ...
	  map_nu_N_g4id_daught_idx[mom] = i;
          if( pdg == 2212 ) {
	    num_n_p++; n_p_g4id_set.insert(g4id); map_n_p[mom].push_back(g4id);   // protons
	    h_trueNeutronEvsProtonE->Fill(part_KE[map_g4id_index[mom]],part_KE[i]);
	    h_trueSignalX->Fill(part_loc.X());
	  }
	...
	if(0) {
          ...
          std::vector<float> recoDirComponents = blipDirection( blip_id[i], map_blipID_hitID, file, model_path );
	...
	if( isProton ) {
	  if( !nearDeath ) {
	    if( !nearCosmic ) {
	      selectedBlips++;
	      h_blipE[0]->Fill(blipE); h_recoVtxDist[0]->Fill(dist); h_Edensity[0]->Fill(density); h_proxTrkDist[0]->Fill(dist2Trk);
	...
	} if( !nearDeath && isProton && !nearCosmic ) trueBlips++;
	//if( p < 0 ) { std::cout<<"p = -9 after cosmic removal\n"; continue; }

	TVector3 part_loc(part_startPointx[p], part_startPointy[p], part_startPointz[p]);
	//std::cout<<"|part_loc-blip_loc|: "<<(part_loc-blip_loc).Mag()<<"\n";
	...
	if( is_n_descent ) {
	  ...
	  // PROTONS
 	  if( p_pdg == 2212 && !part_isPrimary[p] ) {   // any proton
		...
		if( blipE <= 10 && !nearDeath ) gr_signal_E_ds->SetPoint(gr_signal_E_ds->GetN(), blipE, density);
		if( !nearCosmic && !nearDeath && isProton ) {
		  h_blipSignalTrueX->Fill(part_loc.X());
		  signal_blips++; h_foundNeutronE->Fill(part_KE[mom_idx]);
		  h_signalBlipE->Fill(blipE); pr_energy.push_back(trueE);
		  if( map_n_p[mom_g4id][0] == p_g4id ) neutronIDprBlips++;
                  if( map_n_p.at(mom_g4id).size() == 1 ) {   // single protons only
		    single_pr++;
		    h_pr_cos_vs_energy->Fill(trueE, true_cos); h_truePrCosAlign->Fill(true_cos);
		    h_pr_res_vs_trueE->Fill(trueE, res_angle);
		  }
		}
		if( map_n_p.at(mom_g4id).size() == 1 ) { category=1; }   // 1p from primary neutron moms
	...
	if( part_pdg[mom_idx] != 2112 && !(abs(p_pdg) == 11 && part_pdg[mom_idx] == 22) && !(part_isPrimary[p] && p_pdg == 2212) ) {
	  category=7;
          if( isProton && !nearDeath && !nearCosmic ) otherNotGamma++;
        }
	// GAMMAS
	...
	if( 0 && category==7 && isProton && !nearDeath && !nearCosmic ) {
	  if( abs(p_pdg) == 11 && part_pdg[mom_idx] == 22 && part_pdg[grandmom_idx] != 2112 ) { printf("Gamma 'other' process: %24s     Mother PDG: %-10i    Event: %i     G4ID: %i\n", part_process->at(mom_idx).c_str(), part_pdg[grandmom_idx], iEvent, mom_g4id); }
	  else { printf("NON-gamma 'other' process: %20s     PDG: %-10i\n", part_process->at(p).c_str(), part_pdg[p]); }
	}
    ...
    TH1D* h_pre_Eff_vs_E = (TH1D*)h_preFoundNeutronE->Clone("pre_Eff_vs_E");   // preselection
    ...
    TH1D* h_pre_drawn = (TH1D*)h_pre_Eff_vs_E->DrawCopy();  // capture drawn copy

    TH1D* h_selection_Eff_vs_E = (TH1D*)h_foundNeutronE->Clone("selection_Eff_vs_E");   // selection
    ...
    TH1D* h_sel_drawn = (TH1D*)h_selection_Eff_vs_E->DrawCopy("same");  // capture drawn copy
    ...
    TH1D* h_temp1 = (TH1D*)h_blipSignalTrueX->Clone("temp1");
    h_temp1->Sumw2(); h_trueSignalX->Sumw2();
    h_temp1->Divide(h_trueSignalX);
    h_temp1->DrawCopy();
    ...
    " protons\nMean signal proton true energy: "<<pr_energy_mean<<
    ...
    "\n\nEfficiency: "<<neutronIDprBlips/total_nu_Ns*100<<
