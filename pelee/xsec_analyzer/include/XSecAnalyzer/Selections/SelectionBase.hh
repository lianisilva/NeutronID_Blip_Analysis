...
#include "XSecAnalyzer/STV_Tools.hh"//STVTools.hh
...
protected:
  ...
  void SetBranch( MyPointer<T>& u_ptr, S var_name, VarType ) {
    this->SetBranch( u_ptr, var_name );
  }
  // Sets the branch address for output TTree variables managed by this
  // SelectionBase object (one version in case of VarType syntax and the full regular version after)
  template < typename T, typename S >
  void SetBranch( T* var, S var_name, VarType ) {
    this->SetBranch( var, var_name );  // just forward, ignore the enum
  }
  template < typename T, typename S > void SetBranch( T*& var, S var_name )
  {
  ...
  template < typename T, typename S >
    void SetBranch( MyPointer<T>& u_ptr, S var_name )
  {
    T*& address = u_ptr.get_bare_ptr();
    this->SetBranch( address, var_name );
  }
  ...
  //                       ? VariableName
  //                       : selection_name_ + "_" + VariableName;
  ...
  STV_Tools stv_tools_;//STVTools stv_tools_;
