...
void SystematicsCalculator::build_universes( TDirectoryFile& root_tdir ) {
  ...
  std::cout << "ENTERING SYST CALC BUILD UNIVERSES" << std::endl;
...
std::unique_ptr< CovMatrixMap > SystematicsCalculator::get_covariances() const
{
  ...
  while ( config_file >> name >> type ) {
    ...
    if ( type == "sum" ) {
      ...
      make_cov_mat( *this, temp_cov_mat, *detVar_cv_u, *detVar_alt_u, false, false );
...
MeasuredEvents SystematicsCalculator::get_measured_events() const
{
  ...
  const TH1D* ext_hist = data_hists_.at(NFT::kExtBNB).get();   // EXT counts
  ...
  for ( int r = 0; r < num_ordinary_reco_bins_; ++r ) {
    ...
    double ext_only  = ext_hist->GetBinContent(r + 1);
    double total_bkg = (*ext_plus_mc_bkgd)(r, 0);
    double mc_bkg    = total_bkg - ext_only;
    ordinary_data( r, 0 ) = bnb_events;
    std::cout << "  [BIN " << r << "] Data count = " << bnb_events << std::endl;
    std::cout << "[DEBUG] Reco bin " << r
              << " | Total Bkg = " << total_bkg
              << " (EXT = " << ext_only
              << ", MC bkg = " << mc_bkg << ")"
              << std::endl;
