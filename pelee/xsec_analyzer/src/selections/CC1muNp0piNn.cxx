...
// added for naming convention agreement
STV_Tools STVTools;
#define ReturnRecoFV return_reco_FV
#define ReturnTrueFV return_true_FV
#define IsEventMCSignal is_event_mc_signal
#define NEUTRON_TRACK_SCORE_CUT MUON_TRACK_SCORE_CUT   // this is wrong I think
...
void CC1muNp0piNn::define_constants() {
  define_true_FV(21.5,234.85,-95.0,95.0,21.5,966.8);
  define_reco_FV(21.5,234.85,-95.0,95.0,21.5,966.8);
}

void CC1muNp0piNn::compute_true_observables(AnalysisEvent* Event) {
...
void CC1muNp0piNn::compute_reco_observables(AnalysisEvent* Event) {
...
bool CC1muNp0piNn::define_signal(AnalysisEvent* Event) {
  sig_inFV_ = point_inside_FV(this->true_FV(), Event->mc_nu_vx_, Event->mc_nu_vy_, Event->mc_nu_vz_);//this->true_FV()
...
bool CC1muNp0piNn::selection(AnalysisEvent* Event) {
  ...
  sel_reco_vertex_in_FV_ = point_inside_FV(this->reco_FV(), Event->nu_vx_, Event->nu_vy_, Event->nu_vz_);//ReturnRecoFV()
  ...
  for ( int p = 0; p < Event->num_pf_particles_; ++p ) {
    ...
    float track_score = Event->trk_score_v_->at( p );
  ...
  for ( int p = 0; p < Event->num_pf_particles_; ++p ) {
    ...
    float tscore = Event->trk_score_v_->at( p );
...
int CC1muNp0piNn::categorize_event(AnalysisEvent* Event) {
  ...
  bool MCVertexInFV = point_inside_FV(this->true_FV(), Event->mc_nu_vx_, Event->mc_nu_vy_, Event->mc_nu_vz_);//ReturnTrueFV
  if ( !MCVertexInFV ) {
    return kOOFV;
  }
...
void CC1muNp0piNn::define_output_branches() {
...
void CC1muNp0piNn::define_category_map() {
