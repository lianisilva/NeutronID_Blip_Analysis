...
void Blip_n_Np::define_constants() {
  ...
  //this->FV_exact( 0., 256., -116.25, 116.25, 0., 1036.8 );
  //this->FV_buffer( 10., 246.35, -106.5, 106.5, 10., 1026.8 );
  //this->FV_bufferAndDeadZone( 5., 251., -110., 110, 20., 986. );   // need to add DeadZone
  //FV_exact = { 0., 256., -116.25, 116.25, 0., 1036.8 };
  //FV_buffer = { 10., 246.35, -106.5, 106.5, 10., 1026.8 };
  //FV_bufferAndDeadZone = { 5., 251., -110., 110, 20., 986. };   // need to add DeadZone
}
...
bool Blip_n_Np::define_signal( AnalysisEvent* event ) {
  ...
  bool in_fv   = point_inside_FV( this->true_FV(), event->mc_nu_vx_,
...
//          << "\n";
    ...
    for ( int p = 0; p < (int)event->trk_score_v_->size(); ++p ) {
        ...
        if ( !point_inside_FV( this->true_FV(),
        ...
        if ( dist_to_vtx >= 4.0 )                                           continue;//{std::cout<<"Continued after vtx check\n";continue;}
        if ( event->track_llr_pid_score_->at(p) <= 0.2 )                    continue;//{std::cout<<"Continued after PID check\n";continue;}
        if ( trk_length <= 10.0 )                                           continue;//{std::cout<<"Continued after trk length check\n"; continue;}
