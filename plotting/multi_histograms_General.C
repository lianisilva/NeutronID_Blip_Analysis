...
void multi_histograms_General(const char* file1, const char* file2, const char* file3, const char* file4, const char* config_file = "plot_configs.txt") {
	...
	double all = hist_var[0]->Integral(0,Nbins+1); double integrals[numHists];
	  ...
	  integrals[i] = hist_var[i]->Integral(0,Nbins+1);
	    ...
	    "(n, Np) primary neutrons",
            ...
            "primary protons",
            "(n, gamma) all neutrons",
            "other neutron-induced",
            "other interactions",
	    "shower products",
            "cosmics & radiogenics"
        };
	...
	for(int i=1; i<numHists; i++) { legend->AddEntry(hist_var[i], TString::Format("%s = %.1f%%   (%.0f)", legend_labels[i], (integrals[i]/all)*100, integrals[i]), "f"); }
	...
	else if( histName == "byDeadWire" ) { stack_log->SetMinimum(10.0); } else if( histName == "dw" ) { stack_log->SetMinimum(10); }
        else if( histName == "size" ) { stack_log->SetMinimum(3); }
    ...
    outFile->Close();
    std::cout << "\n=== All plots saved to all_plots_output.root ===" << std::endl;
