  ...
  bool  part_isContained[kMaxG4];      // is particle fully contained in active vol
  ...
  int   edep_g4trkid[kMaxEDeps];  // leading G4 track ID
  int   edep_pdg[kMaxEDeps];      // leading G4 track PDG
  int	edep_partid[kMaxEDeps];   // matches to particle index
  ...
  float	hit_driftT[kMaxHits];       // corrected peak time (tick)
  ...
  int   trk_g4id[kMaxTrks];       // G4ID of particle matched to track
  ...
  int   clust_blipid[kMaxClusts];       // blip ID for this cluster (if it was made into one)
  ...
  float blip_dx[kMaxBlips];           // dX [cm] blip's extent along the drift direction (x)
  float blip_dw[kMaxBlips];	      // dW [cm] blip's extent along YZ-plane based on whichever wireplane had the most number of hit wires
  float blip_size[kMaxBlips];	      // sqrt( blip_dX^2 + blip_dW^2 )
  ...
  int   blip_partid[kMaxBlips];	      // matches to particle index
  ...
  int   blip_pl0_clustid[kMaxBlips];  // index of cluster on plane 0 (1st induction)
  int   blip_pl1_clustid[kMaxBlips];  // index of cluster on plane 1 (2nd induction)
  int   blip_pl2_clustid[kMaxBlips];  // index of cluster on plane 2 (collection)
