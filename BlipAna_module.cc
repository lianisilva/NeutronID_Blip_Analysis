...
class BlipAnaTreeDataStruct 
{
  ...
  float nu_nuscore;                   // neutrino score
  int   nu_sliceindex;                // slice index
  ...
  float nu_reco_vtx_x_sce;            // reconstructed vertex X w/SCE correction [cm]
  float nu_reco_vtx_y_sce;            // reconstructed vertex Y w/SCE correction [cm]
  float nu_reco_vtx_z_sce;            // reconstructed vertex Z w/SCE correction [cm]
  ...
  void Clear(){ 
    ...
    */
    ...
    nu_reco_vtx_y           = -999;
    nu_reco_vtx_z           = -999;
    nu_reco_vtx_x_sce       = -999;
    nu_reco_vtx_y_sce       = -999;
    nu_reco_vtx_z_sce       = -999;
  }
  ...
  void MakeTree(){
    ...
    evtTree->Branch("nu_sliceindex",&nu_sliceindex,"nu_sliceindex/I");
    ...
    evtTree->Branch("nu_reco_vtx_x_sce",&nu_reco_vtx_x_sce,"nu_reco_vtx_x_sce/F");
    evtTree->Branch("nu_reco_vtx_y_sce",&nu_reco_vtx_y_sce,"nu_reco_vtx_y_sce/F");
    evtTree->Branch("nu_reco_vtx_z_sce",&nu_reco_vtx_z_sce,"nu_reco_vtx_z_sce/F");
  ...
  ,fData  (nullptr)
{
  ...
  art::ServiceHandle<art::TFileService> tfs;
  fNuSelectionTree = tfs->make<TTree>("NuSelectionTree","nu selection tree");
    ...
    for (const ProxyPfpElem_t &pfp_pxy : pfp_proxy)
    {
      ...
      if ( (PDG == 12) || (PDG == 14) ) 
      {
        ...
        if (vtx.size() == 1)
        {
          ...
          fData->nu_reco_vtx_x_sce = _reco_nu_vtx_sce[0];
          fData->nu_reco_vtx_y_sce = _reco_nu_vtx_sce[1];
          fData->nu_reco_vtx_z_sce = _reco_nu_vtx_sce[2];
        ...
        for (auto pfp : slice_pfp_v)
        {
          ...
          trkscore_v.push_back(trkscore);
          std::cout<<"Track score "<<trkscore<<"\n";
        } // for all PFParticles in the slice
     
        std::cout<<"  - "<<sliceTracks.size()<<" tracks\n";
        std::cout<<"  - "<<sliceShowers.size()<<" showers\n";

        // run selection on this slice
        bool selected = _selectionTool->selectEvent(evt, slice_pfp_v);
        if (selected) std::cout << "SLICE was selected!" << std::endl;
